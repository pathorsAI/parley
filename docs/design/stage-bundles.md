...
|------|-------|---------------------|-----------|
| `discovery.situation` | S・現況 | 對方現在怎麼做這件事：既有流程/系統/供應商、規模與分工 | goal(surface,theirs), relation |
| `discovery.problem` | P・痛點 | 對方**自己說出**的不滿與困難——誰在痛、痛在哪個環節 | risk(theirs), stance |
| `discovery.implication` | I・後果 | 痛不解決的量化代價：錢/時間/風險/機會成本。**必須是客戶認領的**，我方推算的標「薄」 | risk, goal(deep,theirs) |
| `discovery.needpayoff` | N・價值認領 | 對方自己說出「解掉會多好」；理想是他替你講方案價值 | goal(deep,theirs), nextmove |
...
- **幽靈列**（負空間視覺）：slot 未達「實」時，最後一列以虛線框＋淡字顯示 `hint`——一眼掃過去，哪格薄立刻浮出來。點幽靈列 → 建議問法（§5）。
...
- **Live 右欄（S11）**：會議連結 sales 戰線時，右欄情報板的 sales 版升級為缺口板視圖——slot 狀態即時更新（本場新抽的卡以「提案中」樣式先掛上，會後審核才真正入庫，維持 D8 寫入紀律）。現有 sales 情報板的 objection/commitment 追蹤保留為板下方的第二區（互補不衝突）。
...
1. **SPIN 用詞**：板上直接用 S/P/I/N 術語，還是 Pathors 化（現況/痛點/後果/價值認領）？en 對應？
2. **live 板位置**：sales 情報板「升級為」缺口板（本文提案）vs 並列 tab？objection 追蹤區保留與否？
3. **薄/實門檻**：`solidAt=2`、過舊=30 天是拍腦袋值——用你的 AI3/喜來登卡片庫回放校準？
4. **預計時長的家**：掛在「這場會議」（本文提案，連結 dialog 設）vs 掛在 Thread（每場繼承）？
5. **builtin 文案定稿流程**：五階段 slot/hint 文案我先全部起草再一次 review，還是 discovery 先行其餘沿用舊指南？
6. **P1 順序**：本文提「作戰室先、live 後」（風險低、會前會後先有價值）；若你要 live-first（開會當下最痛），P1/P2 對調，成本是先碰 gating 與暫態路徑。
7. `spin-order` eval 的誤報容忍：info 級夠低調嗎，還是 P3 再上？
