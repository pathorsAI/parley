...
**Status: 1.9 in preparation.** The app records in-person meetings, transcribes them live through the hosted relay, syncs them to the account, imports existing audio files and batch-transcribes them, and ships a voice-typing keyboard extension. `ParleyKit` holds the transcript core, ported line-for-line from the desktop's Rust with matching unit tests:
